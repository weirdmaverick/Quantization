...
# ───────────────────────────────────────────────────────────────────────────────
print(f"[Export] Exporting FP32 model to ONNX (dir: {ONNX_FP32_DIR})...")
...
quantize_dynamic(
    ...
    model_output=os.path.join(ONNX_INT8_DIR, "model.onnx"),
    weight_type=WEIGHT_TYPE,
    per_channel=False
)
print(
    f"[Quantization] Completed dynamic weight-only quantization and saved to {ONNX_INT8_DIR}\n")
...
def evaluate_tuned(onnx_dir, run_profile=RUN_PROFILE):
    ...
    # WARNING levels: (0=VERBOSE, 1=INFO, 2=WARNING)
...
# ───────────────────────────────────────────────────────────────────────────────
results = {}
for name, path in [("FP32", ONNX_FP32_DIR), ("Quant", ONNX_INT8_DIR)]:
    ...
    print(
        f"[Compare] {name} → time: {total_t:.3f}s | size: {size_mb:.3f}MB | acc: {acc:.4f}\n"
    )
...
for name in results:
    print(
        f"{name} → time: {results[name]['time']:.3f}s | "
